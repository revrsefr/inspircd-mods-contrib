...
namespace GnuTLS
{
	...
	public:
		...
		gnutls_x509_privkey_t get() const { return key.key; }
	};
	...
	public:
		...
		const gnutls_x509_crt_t* raw() const { return certs.data(); }
	...
	: public Module
{
	...
	struct SNIEntry final
	{
		std::string host;
		std::string profile;
		GnuTLSIOHookProvider* provider = nullptr;
		std::string source;
	};
	...
	std::vector<SNIEntry> sni;
	std::function<void(char*, size_t)> rememberer;

	GnuTLSIOHookProvider* FindProvider(const std::string& name)
	{
		for (const auto& profile : profiles)
		{
			if (insp::equalsci(profile->GetProfile().GetName(), name))
				return profile.get();
		}
		return nullptr;
	}
	...
	{
		sni.clear();
		auto tags = ServerInstance->Config->ConfTags("sni");
		for (const auto& [_, tag] : tags)
		{
			const std::string host = tag->getString("host");
			std::string profile = tag->getString("sslprofile");
			if (profile.empty())
				profile = tag->getString("profile");

			if (host.empty() || profile.empty())
				continue;

			auto* provider = FindProvider(profile);
			if (!provider)
			{
				ServerInstance->Logs.Warning(MODNAME, "Ignoring <sni> tag at {} with unknown sslprofile '{}'", tag->source.str(), profile);
				continue;
			}
...
public:
	bool GetSNIHost(gnutls_session_t sess, std::string& out) const
	{
		std::vector<char> nameBuffer(1);
		size_t nameLength = 0;
		unsigned int nameType = GNUTLS_NAME_DNS;
		...
		out.assign(nameBuffer.data(), nameLength);
		return true;
	}

	const GnuTLS::X509Credentials* FindCredentialsBySNI(const std::string& snihost) const
	{
		for (const auto& entry : sni)
		{
			if (!InspIRCd::Match(snihost, entry.host, ascii_case_insensitive_map))
				continue;

			if (!entry.provider)
				return nullptr;

			return &entry.provider->GetProfile().GetX509Credentials();
		}
		return nullptr;
	}
...
int GnuTLS::X509Credentials::cert_callback(gnutls_session_t sess, const gnutls_datum_t* req_ca_rdn, int nreqs, const gnutls_pk_algorithm_t* sign_algos, int sign_algos_length, gnutls_retr2_st* st)
{
	...
	const GnuTLS::X509Credentials* usecred = &cred;

	auto* mod = static_cast<ModuleSSLGnuTLS*>(thismod);
	if (mod)
	{
		std::string snihost;
		if (mod->GetSNIHost(sess, snihost))
		{
			if (const auto* snicred = mod->FindCredentialsBySNI(snihost))
				usecred = snicred;
		}
	}

	st->ncerts = static_cast<unsigned int>(usecred->certs.size());
	st->cert.x509 = const_cast<gnutls_x509_crt_t*>(usecred->certs.raw());
	st->key.x509 = usecred->key.get();
	st->deinit_all = 0;
