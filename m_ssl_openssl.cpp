...
namespace OpenSSL
{
	...
	public:
		...
		SSL_CTX* GetServerContext() { return ctx.GetRawContext(); }
		const SSL_CTX* GetServerContext() const { return ctx.GetRawContext(); }
	};
...
public:
	...
	const OpenSSL::Profile& GetProfile() const { return profile; }
};
	...
	: public Module
{
	...
	struct SNIEntry final
	{
		std::string host;
		std::string profile;
		OpenSSLIOHookProvider* provider = nullptr;
		std::string source;
	};
	...
	{
		auto* mod = static_cast<ModuleSSLOpenSSL*>(arg);
		return mod ? mod->HandleSNI(ssl) : SSL_TLSEXT_ERR_NOACK;
	}

	int HandleSNI(SSL* ssl)
	{
		const char* name = SSL_get_servername(ssl, TLSEXT_NAMETYPE_host_name);
		if (!name || !*name)
			return SSL_TLSEXT_ERR_NOACK;

		const std::string snihost(name);
		for (const auto& entry : sni)
		{
			...
			SSL_CTX* const newctx = entry.provider->GetProfile().GetServerContext();
			if (SSL_get_SSL_CTX(ssl) != newctx)
	...
	void ConfigureSNI(OpenSSL::Profile& profile)
	{
		SSL_CTX* const ctx = profile.GetServerContext();
		SSL_CTX_set_tlsext_servername_callback(ctx, SNISelectCallback);
		SSL_CTX_set_tlsext_servername_arg(ctx, this);
	}

	OpenSSLIOHookProvider* FindProvider(const std::string& name)
	{
